...
#Amount of points to generate ark
const ARC_POINTS := 8
...
func _ready() -> void:
	#connect card aiming signal from event bus
...
func _get_points() -> Array:
	...
	start.x += (current_card.size.x / 2) #so we start form the center of a card but not from top left corner
