...
func add_shop_modifier(shop: Shop) -> void:
	...
	if not coupons_modifier_value:
		coupons_modifier_value = ModifierValue.create_new_modifier("coupons", ModifierValue.Type.MULTIPLICATIVE)
		coupons_modifier_value.multiplier_add = -1 * discount/100.0
